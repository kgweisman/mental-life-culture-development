...
subtitle: "Exploratory factor analysis using full datasets, Pearson correlations, and oblique transformations"
...
ggsave("../figures/fig03_oblique.pdf")
```
...
```{r cong cis children b, fig.width = 4, fig.asp = 1.4}
# FIGURE 4
...
ggsave("../figures/fig04_oblique.pdf")
```

```{r alt fig 4, fig.width = 4, fig.asp = 0.5}
# FIGURE 4
# fig.asp chosen to keep absolute height of y-axis relatively similar across adults and children
cong_df_children %>%
  mutate(region_A = case_when(
...
```{r alt fig 3, fig.width = 4, fig.asp = 0.5}
# fig.asp chosen to keep absolute height of y-axis relatively similar across adults and children
cong_df_adults %>%
  mutate(region_A = case_when(
    country_A == "US" ~ "SF Bay Area",
...
```{r cong all pairs plot a, fig.width = 9.5, fig.asp = 0.9}
# FIGURE 2
...
ggsave("../figures/fig02_oblique.pdf")
```

```{r cong all pairs plot b, fig.width = 6, fig.asp = 0.8}
# FIGURE 2
cong_lower_lim <- ifelse(min(cong_all_ordered$cong) > -0.05, -0.05, 
                         min(cong_all_ordered$cong))
# cong_plot_colors <- c("red4", "blue4", "darkorchid4", "black")
...
plot_grid(heatmap_comp_fun(
  ...
  ncol = 1, rel_heights = c(2, 1.5), labels = "AUTO")
ggsave("../figures/figS10_oblique.png")
```
### Figure 1, version 1
...
```{r loadings all samples v2a, fig.width = 7, fig.asp = 0.6}
# FIGURE 1, version 2 (included in main text)
loadings_adults %>%
  bind_rows(loadings_children) %>%
...
ggsave("../figures/fig01v2_oblique.png")
ggsave("../figures/fig01v2_oblique.pdf")
```
```{r loadings all samples v2b, fig.width = 4.5, fig.asp = 0.6}
...
loadings_adults %>%
  ...
  # mutate(loading = round(loading, 2)) %>%
  # spread(country, loading)
ggsave("../figures/fig01v2_oblique_adults.png")
```
