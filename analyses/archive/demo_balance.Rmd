...
```{r}
gender_count_adults <- bind_rows(
  d_us_adults %>% distinct(country, subj_id, gender),
  d_gh_adults %>% distinct(country, subj_id, gender),
  d_th_adults %>% distinct(country, subj_id, gender),
  d_ch_adults %>% distinct(country, subj_id, gender),
  d_vt_adults %>% distinct(country, subj_id, gender)) %>% 
  count(country, gender) %>%
  spread(gender, n) %>%
  mutate(country = factor(country, levels_country)) %>%
  arrange(country)
gender_count_adults

gender_count_children <- bind_rows(
  d_us_children %>% distinct(country, subj_id, gender),
  d_gh_children %>% distinct(country, subj_id, gender),
  d_th_children %>% distinct(country, subj_id, gender),
  d_ch_children %>% distinct(country, subj_id, gender),
  d_vt_children %>% distinct(country, subj_id, gender)) %>% 
  count(country, gender) %>%
  spread(gender, n) %>%
  mutate(country = factor(country, levels_country)) %>%
  arrange(country)
gender_count_children
```

```{r}
# ghanaian adults: women only
d_gh_adults_w_f <- d_gh_adults_w %>%
  rownames_to_column("subj_id") %>%
  left_join(d_gh_adults %>%
              distinct(subj_id, gender)) %>%
  filter(gender == "female") %>%
  select(-gender) %>%
  column_to_rownames("subj_id")

reten_fun_par(d_gh_adults_w_f)
heatmap_fun(fa_fun(d_gh_adults_w_f, n = 3, chosen_rot = "oblimin"))
```

```{r}
# thai adults: women only
d_th_adults_w_f <- d_th_adults_w %>%
  rownames_to_column("subj_id") %>%
  left_join(d_th_adults %>%
              distinct(subj_id, gender)) %>%
  filter(gender == "female") %>%
  select(-gender) %>%
  column_to_rownames("subj_id")

reten_fun_par(d_th_adults_w_f)
heatmap_fun(fa_fun(d_th_adults_w_f, n = 3, chosen_rot = "oblimin"))
```

```{r}
# ni-vanuatu adults: women only
d_vt_adults_w_f <- d_vt_adults_w %>%
  rownames_to_column("subj_id") %>%
  left_join(d_vt_adults %>%
              distinct(subj_id, gender)) %>%
  filter(gender == "female") %>%
  select(-gender) %>%
  column_to_rownames("subj_id")

reten_fun_par(d_vt_adults_w_f)
heatmap_fun(fa_fun(d_vt_adults_w_f, n = 2, chosen_rot = "oblimin"))
...
```{r}
age_adults <- bind_rows(
  d_us_adults %>% distinct(country, subj_id, age),
  d_gh_adults %>% distinct(country, subj_id, age),
  d_th_adults %>% distinct(country, subj_id, age),
  d_ch_adults %>% distinct(country, subj_id, age),
  d_vt_adults %>% distinct(country, subj_id, age)) %>%
  mutate(country = factor(country, levels = levels_country))
```

```{r}
age_adults %>%
  ggplot(aes(x = age, fill = country, color = country)) +
...
```{r}
# gender counts after age filter
gender_count_adults <- bind_rows(
  d_us_adults %>% distinct(country, subj_id, gender, age),
  d_gh_adults %>% distinct(country, subj_id, gender, age),
  d_th_adults %>% distinct(country, subj_id, gender, age),
  d_ch_adults %>% distinct(country, subj_id, gender, age),
  d_vt_adults %>% distinct(country, subj_id, gender, age)) %>%
...
```{r}
demobal_fun <- function(df, ref_df = NULL,
                        min_age = NA_real_, max_age = NA_real_,
                        n_female = NA_real_, n_male = NA_real_) {
  ...
  if (!is.na(min_age)) {
    ref_df <- ref_df %>% filter(age >= min_age | is.na(age))
  }
  
  if (!is.na(max_age)) {
    ref_df <- ref_df %>% filter(age <= max_age | is.na(age))
  }
  ...
  female_subj_id <- ref_df %>%
    filter(gender == "female") %>%
    distinct(subj_id)
  
  if (!is.na(n_female)) {
    female_subj_id <- female_subj_id %>% sample_n(n_female, replace = F)
  }
  ...
  if (!is.na(n_male)) {
    male_subj_id <- male_subj_id %>% sample_n(n_male, replace = F)
  }
  
  res <- df %>%
    filter(subj_id %in% female_subj_id$subj_id |
             subj_id %in% male_subj_id$subj_id)
...
```{r}
d_us_adults <- demobal_fun(d_us_adults, 
                           min_age = 17, max_age = 65)
                           # n_female = min(gender_count_adults$female),
                           # n_male = min(gender_count_adults$male))

d_us_adults_w <- demobal_fun(d_us_adults_w, d_us_adults,
                             min_age = 17, max_age = 65)
                             # n_female = min(gender_count_adults$female),
                             # n_male = min(gender_count_adults$male))

d_gh_adults <- demobal_fun(d_gh_adults, 
                           min_age = 17, max_age = 65)
                           # n_female = min(gender_count_adults$female),
                           # n_male = min(gender_count_adults$male))

d_gh_adults_w <- demobal_fun(d_gh_adults_w, d_gh_adults,
                             min_age = 17, max_age = 65)
                             # n_female = min(gender_count_adults$female),
                             # n_male = min(gender_count_adults$male))

d_th_adults <- demobal_fun(d_th_adults, 
                           min_age = 17, max_age = 65)
                           # n_female = min(gender_count_adults$female),
                           # n_male = min(gender_count_adults$male))

d_th_adults_w <- demobal_fun(d_th_adults_w, d_th_adults,
                             min_age = 17, max_age = 65)
                             # n_female = min(gender_count_adults$female),
                             # n_male = min(gender_count_adults$male))

d_ch_adults <- demobal_fun(d_ch_adults, 
                           min_age = 17, max_age = 65)
                           # n_female = min(gender_count_adults$female),
                           # n_male = min(gender_count_adults$male))

d_ch_adults_w <- demobal_fun(d_ch_adults_w, d_ch_adults,
                             min_age = 17, max_age = 65)
                             # n_female = min(gender_count_adults$female),
                             # n_male = min(gender_count_adults$male))

d_vt_adults <- demobal_fun(d_vt_adults, 
                           min_age = 17, max_age = 65)
                           # n_female = min(gender_count_adults$female),
                           # n_male = min(gender_count_adults$male))

d_vt_adults_w <- demobal_fun(d_vt_adults_w, d_vt_adults,
                             min_age = 17, max_age = 65)
                             # n_female = min(gender_count_adults$female),
                             # n_male = min(gender_count_adults$male))
```
...
```{r}
# d_us_children <- demobal_fun(d_us_children, 
#                             n_female = min(gender_count_children$female),
#                             n_male = min(gender_count_children$male))
# 
# d_us_children_w <- demobal_fun(d_us_children_w, d_us_children,
#                               n_female = min(gender_count_children$female),
#                               n_male = min(gender_count_children$male))
# 
# d_gh_children <- demobal_fun(d_gh_children, 
#                             n_female = min(gender_count_children$female),
#                             n_male = min(gender_count_children$male))
# 
# d_gh_children_w <- demobal_fun(d_gh_children_w, d_gh_children,
#                               n_female = min(gender_count_children$female),
#                               n_male = min(gender_count_children$male))
# 
# d_th_children <- demobal_fun(d_th_children, 
#                             n_female = min(gender_count_children$female),
#                             n_male = min(gender_count_children$male))
# 
# d_th_children_w <- demobal_fun(d_th_children_w, d_th_children,
#                               n_female = min(gender_count_children$female),
#                               n_male = min(gender_count_children$male))
# 
# d_ch_children <- demobal_fun(d_ch_children, 
#                             n_female = min(gender_count_children$female),
#                             n_male = min(gender_count_children$male))
# 
# d_ch_children_w <- demobal_fun(d_ch_children_w, d_ch_children,
#                               n_female = min(gender_count_children$female),
#                               n_male = min(gender_count_children$male))
# 
# d_vt_children <- demobal_fun(d_vt_children, 
#                             n_female = min(gender_count_children$female),
#                             n_male = min(gender_count_children$male))
# 
# d_vt_children_w <- demobal_fun(d_vt_children_w, d_vt_children,
#                               n_female = min(gender_count_children$female),
#                               n_male = min(gender_count_children$male))
```
...
bind_rows(d_us_adults, d_gh_adults, d_th_adults, d_ch_adults, d_vt_adults) %>%
  mutate(country = factor(country, levels = levels_country)) %>%
  distinct(country, subj_id, gender) %>%
  count(country, gender) %>%
  group_by(country) %>%
...
if (file.exists("../results/pa_outcomes_dist_adults_demobal.RDS")) {
  
  pa_outcomes_dist_adults <- readRDS("../results/pa_outcomes_dist_adults_demobal.RDS")
  ...
  saveRDS(pa_outcomes_dist_adults, file = "../results/pa_outcomes_dist_adults_demobal.RDS")
}
...
if (file.exists("../results/cong_df_adults_oblique_demobal.RDS")) {
  
  cong_df_adults <- readRDS("../results/cong_df_adults_oblique_demobal.RDS")
  ...
  saveRDS(cong_df_adults, file = "../results/cong_df_adults_oblique_demobal.RDS")
}
```
...
cong_plot_fun(cong_df = cong_df_adults, which_country = "US") +
  ...
  # ylim(NA, 1) +
  labs(x = NULL)
ggsave("../figures/fig03_oblique_demobal.png")
```
...
cong_plot_fun(cong_df = cong_df_adults %>%
              ...
              which_country = "Ghana") +
  ylim(min_cong_adults$min_cong, 1)
ggsave("../figures/figS01_oblique_demobal.png")
```
...
cong_plot_fun(cong_df = cong_df_adults, 
  ...
  ylim(min_cong_adults$min_cong, 1)
ggsave("../figures/figS02_oblique_demobal.png")
```
...
cong_plot_fun(cong_df = cong_df_adults, 
              which_country = "China") +
  ylim(min_cong_adults$min_cong, 1)
ggsave("../figures/figS03_oblique_demobal.png")
```
...
cong_plot_fun(cong_df = cong_df_adults %>%
  ...
  ylim(min_cong_adults$min_cong, 1)
ggsave("../figures/figS04_oblique_demobal.png")
```
...
if (file.exists("../results/pa_outcomes_dist_children_demobal.RDS")) {
  
  pa_outcomes_dist_children <- readRDS("../results/pa_outcomes_dist_children_demobal.RDS")
  ...
  saveRDS(pa_outcomes_dist_children, file = "../results/pa_outcomes_dist_children_demobal.RDS")
}
...
factor_names_children <- data.frame(factor = c(colnames(efa_us_children$loadings),
  ...
  mutate(factor_descript = recode(factor,
                                  ...
                                  thCHILDREN_F2 = "Mind-like",
                                  thCHILDREN_F3 = "Heart-like, negative",
                                  chCHILDREN_F1 = "Mind-like",
                                  chCHILDREN_F2 = "Heart-like",
                                  chCHILDREN_F3 = "Body-like 1",
                                  chCHILDREN_F4 = "Body-like 2",
                                  vtCHILDREN_F1 = "Body-like",
                                  vtCHILDREN_F2 = "Heart-like",
                                  vtCHILDREN_F3 = "Mind-like"),
         factor_labdescript = paste(gsub(".*_F", "F", factor),
                                    factor_descript, sep = ": "))
```
...
if (file.exists("../results/cong_df_children_oblique_demobal.RDS")) {
  
  cong_df_children <- readRDS("../results/cong_df_children_oblique_demobal.RDS")
  ...
  saveRDS(cong_df_children, file = "../results/cong_df_children_oblique_demobal.RDS")
}
```
...
ggsave("../figures/fig04_oblique_demobal.png")
```
...
cong_all_ordered %>%
  ...
  labs(x = NULL, y = NULL, fill = expression(italic(r[c])))
ggsave("../figures/fig02_oblique_demobal.png")
```
...
ggsave("../figures/fig02_oblique_jaccard_demobal.png")
```
...
ggsave("../figures/fig01_oblique_demobal.png")
```
...
ggplot(rect_df %>%
  ...
  theme(panel.spacing.x = unit(c(rep(0.2, 4), 1, rep(0.2, 4)), "line"),
        ...
        legend.position = "bottom")
# ggsave("../figures/fig01v2_oblique_demobal.png")
```
