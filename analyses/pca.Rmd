...
In this notebook we conduct principal dimensions analyses (PCAs) on the datasets for our studies of concepts of mental life, in which each participants judged the various mental capacities of a particular target entity. We analyze datasets for adults and children from each of our five field sites: the US, Ghana, Thailand, China, and Vanuatu. 
...
```{r wide}
# make wide-form datasets for PCA: adults
...
# make wide-form datasets for PCA: children
...
# NOTE: there appears to be some unreliability/non-reproducibility in results, especially for vt adults, which I don't understand -- so here's the distribution over outcomes of parallel analysis with 100 iterations. We'll choose the median number of dimensions.
...
pa_outcomes_dist_adults %>%
  ...
  scale_x_continuous(limits = c(1, max(pa_outcomes_dist_adults) + 1),
                     breaks = seq(0, 100, 1)) +
  labs(x = "Number of dimensions suggested by fa.parallel()")
```
...
factor_names_adults <- data.frame(
  factor = c(colnames(pca_us_adults$loadings),
             colnames(pca_gh_adults$loadings),
             colnames(pca_th_adults$loadings),
             colnames(pca_ch_adults$loadings),
             colnames(pca_vt_adults$loadings))) %>%
...
if (chosen_reten == "ncomp") {
  factor_names_adults <- factor_names_adults %>%
    mutate(factor_descript = recode(factor,
                                    usADULTS_F1 = "Body-like",
                                    usADULTS_F2 = "Mind-like",
                                    ghADULTS_F1 = "Mind-like",
                                    ghADULTS_F2 = "Body-like",
                                    thADULTS_F1 = "Body-like",
                                    thADULTS_F2 = "Mind-like",
                                    chADULTS_F1 = "Body-like",
                                    chADULTS_F2 = "Mind-like",
                                    vtADULTS_F1 = "Mind-like",
                                    vtADULTS_F2 = "Body-like"))
} else if (chosen_reten == "nfact") {
  factor_names_adults <- factor_names_adults %>%
  mutate(factor_descript = recode(factor,
                                  usADULTS_F1 = "Body",
                                  usADULTS_F2 = "Heart",
...
factor_names_adults <- factor_names_adults %>%
  mutate(factor_labdescript = paste(gsub(".*_F", "F", factor),
                                    factor_descript, sep = ": "))
```
...
plot_grid(heatmap_fun(pca_us_adults, 
...
if (file.exists("../results/cong_df_adults_pca.RDS")) {
  ...
  dimensions <- levels(factor(loadings_adults$factor))
...
```{r}
# choose which to use: done above for adults
```
...
factor_names_children <- data.frame(
  factor = c(colnames(pca_us_children$loadings),
             colnames(pca_gh_children$loadings),
             colnames(pca_th_children$loadings),
             colnames(pca_ch_children$loadings),
             colnames(pca_vt_children$loadings))) %>%
...
if (chosen_reten == "ncomp") {
  factor_names_children <- factor_names_children %>%
    mutate(factor_descript = recode(factor,
                                    usCHILDREN_F1 = "Body-like",
                                    usCHILDREN_F2 = "Mind-like",
                                    ghCHILDREN_F1 = "Body-like",
                                    ghCHILDREN_F2 = "Mind-like",
...
plot_grid(heatmap_fun(pca_us_children, 
...
if (file.exists("../results/cong_df_children_pca.RDS")) {
  ...
  for (k in levels_country) {
    ...
    dimensions_children <- levels(factor(loadings_children$factor[
      loadings_children$country == k]))
    dimensions_adults <- levels(factor(loadings_adults$factor[
      loadings_adults$country == k]))
    
    for (i in dimensions_children) {
      for (j in dimensions_adults) {
        cname <- paste(i, j, sep = ".")
        temp <- bs_children %>%
          mutate(cong = map_dbl(strap, ~lsa::cosine(as.data.frame(.x)[,i],
                                                    as.data.frame(.x)[,j])))
    ...
    rm(i, j, cname, temp, dimensions_children, dimensions_adults)
...
if (chosen_reten == "ncomp") {
  cong_all_fig <-  cong_all_fig +
    # body-like dimensions
    annotate("rect", xmin = 0.5, xmax = 10.5, ymin = 10.5, ymax = 20.5,
             color = cong_plot_colors[2], size = 1.5, alpha = 0) +
    # mind-like dimensions
    annotate("rect", xmin = 10.5, xmax = 20.5, ymin = 0.5, ymax = 10.5,
...
plot_grid(heatmap_comp_fun(efa_list = list(pca_us_adults, pca_us_children), padding = F),
...
plot_grid(heatmap_comp_fun(efa_list = list(pca_gh_adults, pca_gh_children), padding = F),
          dev_cong_plot_fun(cong_df_children, which_country = "Ghana", padding = T),
          ncol = 1, rel_heights = c(2, 1.5), labels = "AUTO")
ggsave("../figures/figS06_pca.png")

plot_grid(heatmap_comp_fun(efa_list = list(pca_th_adults, pca_th_children), padding = F),
          dev_cong_plot_fun(cong_df_children, which_country = "Thailand", padding = T),
          ncol = 1, rel_heights = c(2, 1.5), labels = "AUTO")
ggsave("../figures/figS07_pca.png")

plot_grid(heatmap_comp_fun(efa_list = list(pca_ch_adults, pca_ch_children), padding = F),
          dev_cong_plot_fun(cong_df_children, which_country = "China", padding = T),
          ncol = 1, rel_heights = c(2, 1.5), labels = "AUTO")
ggsave("../figures/figS08_pca.png")

plot_grid(heatmap_comp_fun(efa_list = list(pca_vt_adults, pca_vt_children), padding = F),
...
dom_dimensions_all <- loadings_all %>%
