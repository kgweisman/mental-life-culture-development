...
# US -----
files_us <- list.files("../raw data/children/USA_BG_Data CSVs/", 
...
subj_key_us <- d_raw_us %>%
  ...
  mutate(subj_id = paste("us", "children", 
...
if (nrow(d_us_dup) > 0) { print("DUPLICATES IN US CHILDREN") }
...
d_us <- d_us %>%
  ...
  select(country, subj_id, gender, age,
         ...
         character_cat, character_orig, target, sequence, 
...
# Ghana (Fante) -----
files_gh <- list.files("../raw data/children/GHA_BG_Children (Fante) CSV files/", 
                       pattern = "*.csv", full.names = T)
...
subj_key_gh <- d_raw_gh %>%
  ...
  distinct(subj_id_original, target) %>%
  mutate(target = clean_char_fun(target)) %>%
  mutate(subj_id = paste("gh", "children", 
                         str_pad(1:nrow(.), width = 3, side = "left", pad = "0"),
                         target,
                         sep = "_"))
...
if (nrow(d_gh_dup) > 0) { print("DUPLICATES IN GHANA CHILDREN") }
...
# Ghana (English) -----
files_gh_eng <- list.files("../raw data/children/GHA_BG_Children (English) CSV files/", 
                       pattern = "*.csv", full.names = T)

d_raw_gh_eng <- plyr::ldply(files_gh_eng, read_csv) %>%
  rename(target = character) %>%
  filter(!is.na(target), !is.na(sequence))

subj_key_gh_eng <- d_raw_gh_eng %>%
  rename(subj_id_original = `subject ID (name)`) %>%
  distinct(subj_id_original, target) %>%
  mutate(target = clean_char_fun(target)) %>%
  mutate(subj_id = paste("gh", "english", "children", 
                         str_pad(1:nrow(.), width = 3, side = "left", pad = "0"),
                         target,
                         sep = "_"))

d_gh_eng <- d_raw_gh_eng %>%
  rename(subj_id_original = `subject ID (name)`,
         dot = date) %>%
  # clean up target
  mutate(target = clean_char_fun(target)) %>%
  # switch to anonymized subject ids
  left_join(subj_key_gh_eng) %>%
  select(-subj_id_original) %>%
  # get dot
  mutate_at(vars(dot), # no dob
            funs(parse_date_time2(as.character(.), 
                                  orders = "dby",
                                  cutoff_2000 = 18L))) %>%
  # recode gender
...
# check for duplicates
d_gh_eng_dup <- d_gh_eng %>%
  filter(trial == "1") %>%
  count(subj_id) %>%
  filter(n > 1)

if (nrow(d_gh_eng_dup) > 0) { print("DUPLICATES IN GHANA (ENGLISH) CHILDREN") }

d_gh_eng <- d_gh_eng %>%
  # drop identifying info
  select(-dot) %>% # no dob
  # add country
  mutate(country = "Ghana (English)") %>%
  # reorder variables
  select(country, subj_id, gender, #age,
         experimenter, location,
         character_cat, character_orig, target, sequence, 
...
# Thailand -----
files_th <- list.files("../raw data/children/THL_BG_Children CSV files/", 
                       pattern = "*.csv", full.names = T)

d_raw_th <- plyr::ldply(files_th, read_csv) %>%
  rename(target = character) %>%
  # fill in missing data from particpiant log
  mutate(target = case_when(grepl("finding", 
                                  tolower(`subject id`)) ~ "dogs",
                            TRUE ~ target)) %>% 
  filter(!is.na(target), !is.na(sequence))
...
subj_key_th <- d_raw_th %>%
  ...
  distinct(subj_id_original, target) %>%
  mutate(target = clean_char_fun(target)) %>%
  mutate(subj_id = paste("th", "children", 
                         str_pad(1:nrow(.), width = 3, side = "left", pad = "0"),
                         target,
                         sep = "_"))
...
if (nrow(d_th_dup) > 0) { print("DUPLICATES IN THAILAND CHILDREN") }
...
# China -----
files_ch <- list.files("../raw data/children/CHN_BEETLES_CSV Files/", 
                       pattern = "*.csv", full.names = T)
...
subj_key_ch <- d_raw_ch %>%
  ...
  distinct(subj_id_original, target) %>%
  mutate(target = clean_char_fun(target)) %>%
  mutate(subj_id = paste("ch", "children", 
                         str_pad(1:nrow(.), width = 3, side = "left", pad = "0"),
                         target,
                         sep = "_"))
...
if (nrow(d_ch_dup) > 0) { print("DUPLICATES IN CHINA CHILDREN") }
...
# Vanuatu -----
files_vt <- list.files("../raw data/children/VUT_BG_Children CSV files/", 
                       pattern = "*.csv", full.names = T)
...
subj_key_vt <- d_raw_vt %>%
  ...
  distinct(subj_id_original, target) %>%
  mutate(target = clean_char_fun(target)) %>%
  mutate(subj_id = paste("vt", "children", 
                         str_pad(1:nrow(.), width = 3, side = "left", pad = "0"),
                         target,
                         sep = "_"))
...
if (nrow(d_vt_dup) > 0) { print("DUPLICATES IN VANUATU CHILDREN") }
...
write_csv(subj_key, "../raw data/children/subject_key_children.csv")
write_csv(d_us, "../data/d_us_children.csv")
write_csv(d_gh, "../data/d_gh_children.csv")
write_csv(d_gh_eng, "../data/d_gh_eng_children.csv")
write_csv(d_th, "../data/d_th_children.csv")
write_csv(d_ch, "../data/d_ch_children.csv")
write_csv(d_vt, "../data/d_vt_children.csv")
